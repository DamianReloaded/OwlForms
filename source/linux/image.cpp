...
bool image::load (const std::string& _filepath)
{
    ...
    if (m_imp->m_pixmap!=0) XFreePixmap(control::implementation::m_xlib_display, m_imp->m_pixmap);
    ...
    m_imp->m_ximage = XCreateImage(display, CopyFromParent, depth, ZPixmap, 0, (char*)&m_imp->m_buffer[0], m_imp->m_width, m_imp->m_height, bitmap_pad, bytes_per_line);
    m_imp->m_pixmap = XCreatePixmap(display, XDefaultRootWindow(display), m_imp->m_width, m_imp->m_height, depth);
